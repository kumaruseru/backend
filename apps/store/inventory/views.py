"""
Store Inventory - Production-Ready API Views.
...
class StockListView(generics.ListAPIView):
    """List all stock items with filtering."""
    permission_classes = [permissions.IsAdminUser]
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter('warehouse_id', int),
            OpenApiParameter('status', str, enum=['low', 'out', 'in']),
...
class StockTransferView(APIView):
    ...
    def post(self, request):
        ...
        try:
            from_stock, to_stock = InventoryService.transfer_stock(
            ...
            return Response({
                'success': True,
                'from_stock': StockItemSerializer(from_stock).data,
                'to_stock': StockItemSerializer(to_stock).data
            })
        except DomainException as e:
            return Response(e.to_dict(), status=e.http_status)
...
class LowStockView(generics.ListAPIView):
    ...
    @extend_schema(
        parameters=[OpenApiParameter('warehouse_id', int)],
        tags=['Inventory - Stock']
    )
    def get(self, request, *args, **kwargs):
        return super().get(request, *args, **kwargs)
...
class MovementListView(generics.ListAPIView):
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter('product_id', str),
            OpenApiParameter('warehouse_id', int),
...
class InventoryCountCreateView(APIView):
    ...
    @extend_schema(
        request=InventoryCountCreateSerializer,
        responses={201: InventoryCountSerializer},
    ...
    def post(self, request):
        ...
        count = InventoryService.create_inventory_count(
            name=data['name'],
...
class InventoryCountDetailView(APIView):
    ...
    @extend_schema(
        responses={200: InventoryCountSerializer},
        tags=['Inventory - Counts']
    )
    def get(self, request, count_id):
        try:
            count = InventoryCount.objects.prefetch_related(
                'items__stock__product'
            ).get(id=count_id)
...
class InventoryCountStartView(APIView):
    """Start an inventory count session."""
    permission_classes = [permissions.IsAdminUser]
...
class InventoryCountCompleteView(APIView):
    """Complete an inventory count and apply adjustments."""
    permission_classes = [permissions.IsAdminUser]
...
class InventoryCountItemUpdateView(APIView):
    ...
    @extend_schema(
        request=InventoryCountItemUpdateSerializer,
        responses={200: InventoryCountItemSerializer},
        tags=['Inventory - Counts']
    )
    def post(self, request, item_id):
        ...
        try:
            item = InventoryService.update_count_item(
                count_item_id=item_id,
...
class StatisticsView(APIView):
    ...
    @extend_schema(
        parameters=[OpenApiParameter('warehouse_id', int)],
        responses={200: InventoryStatisticsSerializer},
