...
class InventoryService:
    ...
    Handles stock operations, transfers, alerts, and reporting.
    """
    ...
    def get_or_create_stock(product_id: UUID, warehouse_id: int = None) -> StockItem:
        """Get or create stock item for a product."""
        defaults = {'quantity': 0}
    ...
    def process_return(
        ...
        """
        Process return and add back to stock.
        """
        stock = InventoryService.get_stock(product_id, warehouse_id)
    ...
    def get_low_stock_items(warehouse_id: int = None, limit: int = 50) -> list:
        ...
        queryset = StockItem.objects.select_related('product', 'warehouse').filter(
    ...
    def get_pending_alerts(warehouse_id: int = None, limit: int = 50) -> list:
        ...
        queryset = StockAlert.objects.select_related('stock__product').filter(
            is_resolved=False
        )
    ...
    def resolve_alert(alert_id: int, user=None, notes: str = '') -> StockAlert:
        ...
        try:
            ...
            raise NotFoundError(message='Không tìm thấy cảnh báo')
    ...
    def update_count_item(
        ...
        try:
            ...
            raise NotFoundError(message='Không tìm thấy item')
    ...
    def get_statistics(warehouse_id: int = None) -> Dict[str, Any]:
        ...
        total_value = sum(
            (item.unit_cost or item.product.price) * item.quantity
            for item in queryset.select_related('product')
        )
        
        stats = queryset.aggregate(
            total_products=Count('id'),
            in_stock=Count('id', filter=Q(quantity__gt=F('reserved_quantity'))),
        ...
        movement_stats = movements_today.aggregate(
            movements_today=Count('id'),
            items_sold=Sum('quantity_change', filter=Q(reason='sale')),
            items_received=Sum('quantity_change', filter=Q(reason='purchase'))
        )
    ...
    def get_movement_summary(days: int = 30, warehouse_id: int = None) -> Dict[str, Any]:
        ...
        summary = queryset.values('reason').annotate(
            count=Count('id'),
