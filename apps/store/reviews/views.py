"""
Store Reviews - Production-Ready API Views.
"""
...
class ProductReviewListView(APIView):
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter('rating', int, enum=[1, 2, 3, 4, 5]),
            OpenApiParameter('verified_only', bool),
            OpenApiParameter('with_images', bool),
            OpenApiParameter('sort', str, enum=['recent', 'helpful', 'rating_high', 'rating_low']),
    ...
    def get(self, request, product_id):
        result = ReviewService.get_product_reviews(
            product_id=product_id,
            rating=request.query_params.get('rating'),
            verified_only=request.query_params.get('verified_only') == 'true',
            with_images=request.query_params.get('with_images') == 'true',
        ...
        return Response({
            'reviews': ReviewListSerializer(result['reviews'], many=True).data,
            'total': result['total'],
    ...
    def get(self, request, review_id):
        try:
            review = Review.objects.prefetch_related(
                'images', 'replies'
            ).get(id=review_id, is_approved=True)
...
class AdminReviewListView(generics.ListAPIView):
    ...
    @extend_schema(
        parameters=[OpenApiParameter('status', str, enum=['pending', 'approved', 'rejected'])],
        tags=['Reviews - Admin']
    )
    def get(self, request, *args, **kwargs):
        return super().get(request, *args, **kwargs)
...
class AdminReplyView(APIView):
    """Admin: Add official reply to a review."""
    permission_classes = [permissions.IsAdminUser]
